...
CREATE TABLE IF NOT EXISTS records_demo.owners (
  ...
  can_read_restricted  BOOLEAN DEFAULT FALSE
);

-- Records = the protected rows. classification drives the gateway's step-up.
CREATE TABLE IF NOT EXISTS records_demo.records (
  record_id     TEXT PRIMARY KEY,                   -- e.g. REC-1001
  display_name  TEXT NOT NULL,
  dob           DATE,
  email         TEXT,
  classification TEXT DEFAULT 'public',             -- 'restricted' -> gateway forces MFA step-up on read
